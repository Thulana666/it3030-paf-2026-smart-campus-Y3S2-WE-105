        ...
        return (
          <div key={b.id} className="bk-card glass" style={{ 
            ...
            <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '8px', borderTop: '1px solid rgba(0,0,0,0.05)', paddingTop: '1rem' }}>
              {/* User Edit Actions (PENDING or APPROVED) */}
              {user?.role !== 'ADMIN' && currentUserId === b.studentId && (b.status === 'PENDING' || b.status === 'REJECTED') && (
                <button
              ...
              {/* User Cancel Action (PENDING or APPROVED) */}
              ...
              {/* User Delete Action (for CANCELLED or REJECTED) */}
              {user?.role !== 'ADMIN' && currentUserId === b.studentId && (b.status === 'CANCELLED' || b.status === 'REJECTED') && (
                <button
                  className="btn btn-sm"
                  onClick={() => handleCancel(b.id)}
                  disabled={cancellingId === b.id}
