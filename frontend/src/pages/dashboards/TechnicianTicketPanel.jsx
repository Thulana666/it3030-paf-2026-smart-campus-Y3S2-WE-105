...
const TechnicianTicketPanel = ({
  ...
  const handleSendMessage = async (e) => {
    ...
    setSendingMsg(true);
    try {
      const comment = await ticketService.addTechnicianComment(
        ticket.id,
        newMessage.trim(),
      );
  ...
  const handleUpdateStatus = async () => {
    if (
      ...
      resNotes === (ticket.resolutionNotes || "")
    )
      return;
      ...
      <div className="tech-panel">
        ...
        <div className="tech-panel-header">
          ...
          <div className="tech-panel-badges">
            <span className={`ticket-priority ${priorityMeta.cls}`}>
              {priorityMeta.label}
            </span>
            <span className={`ticket-status ${statusMeta.cls}`}>
              {statusMeta.label}
            </span>
          </div>
        </div>
        ...
        <div className="tech-panel-tabs">
          ...
          >
            ...
            Chat{" "}
            {comments.length > 0 && (
              <span className="tech-panel-tab-count">{comments.length}</span>
            )}
          </button>
          <button
              ...
              <div className="tech-chat-container">
                ...
                <div className="tech-chat-student-banner">
                  ...
                  <div>
                    ...
                    <div className="tech-chat-student-sub">
                      Ticket creator · {timeAgo(ticket.createdAt)}
                    </div>
                  </div>
                </div>
                    ...
                    comments.map((comment) => {
                      ...
                      return (
                        ...
                        >
                          {!isMine && (
                            <div
                              className={`tech-avatar ${isTech ? "tech-avatar-tech" : "tech-avatar-student"}`}
                            >
                              {(comment.authorName || "?")
                                .charAt(0)
                                .toUpperCase()}
                          ...
                          <div className="tech-chat-bubble-wrap">
                            <div className="tech-chat-bubble-meta">
                              <span>
                                {isMine ? "You" : comment.authorName || "User"}
                              </span>
                              <span>{timeAgo(comment.createdAt)}</span>
                            </div>
                            <div
                              className={`tech-chat-bubble ${isMine ? "tech-chat-bubble-tech" : "tech-chat-bubble-student"}`}
                            >
                              {comment.content}
                            </div>
                          </div>
                ...
                {!isClosed ? (
                  <form
                    className="tech-chat-input-wrap"
                  ...
                  <div className="tech-chat-closed-notice">
                    ...
                    This ticket is {ticket.status.toLowerCase()} — no new
                    messages.
                  </div>
                )}
              </div>
            ...
            <div className="tech-sidebar-card">
              ...
              {isClosed ? (
                <div className="tech-sidebar-closed">
                  ...
                  <p>
                    Ticket is <strong>{ticket.status}</strong>
                  </p>
                  {ticket.resolutionNotes && (
      ...
      {showCloseModal && (
        ...
        >
          <div className="tech-modal" onClick={(e) => e.stopPropagation()}>
            ...
            <p>
              This will mark the ticket as <strong>CLOSED</strong> and send a
              notification to{" "}
              <strong>{ticket.createdByName || "the student"}</strong>. This
